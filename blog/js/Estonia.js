...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Estonia"
        var publicacionesEstonia = data.filter(function(publicacion) {
            return publicacion.Pais === 'Estonia';
        });

        // Manejar los datos filtrados
        if (publicacionesEstonia.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesEstonia.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Estonia"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Estonia".</p>');
        }
    },
