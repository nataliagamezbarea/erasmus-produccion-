...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Malta"
        var publicacionesMalta = data.filter(function(publicacion) {
            return publicacion.Pais === 'Malta';
        });

        // Manejar los datos filtrados
        if (publicacionesMalta.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesMalta.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Malta"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Malta".</p>');
        }
    },
