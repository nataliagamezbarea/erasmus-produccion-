...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Italia"
        var publicacionesItalia = data.filter(function(publicacion) {
            return publicacion.Pais === 'Italia';
        });

        // Manejar los datos filtrados
        if (publicacionesItalia.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesItalia.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Italia"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Italia".</p>');
        }
    },
