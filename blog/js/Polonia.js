...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Polonia"
        var publicacionesPolonia = data.filter(function(publicacion) {
            return publicacion.Pais === 'Polonia';
        });

        // Manejar los datos filtrados
        if (publicacionesPolonia.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesPolonia.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Polonia"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Polonia".</p>');
        }
    },
