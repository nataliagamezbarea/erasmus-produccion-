...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Finlandia"
        var publicacionesFinlandia = data.filter(function(publicacion) {
            return publicacion.Pais === 'Finlandia';
        });

        // Manejar los datos filtrados
        if (publicacionesFinlandia.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesFinlandia.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Finlandia"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Finlandia".</p>');
        }
    },
