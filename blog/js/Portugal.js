...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Portugal"
        var publicacionesPortugal = data.filter(function(publicacion) {
            return publicacion.Pais === 'Portugal';
        });

        // Manejar los datos filtrados
        if (publicacionesPortugal.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesPortugal.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Portugal"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Portugal".</p>');
        }
    },
