...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Irlanda"
        var publicacionesIrlanda = data.filter(function(publicacion) {
            return publicacion.Pais === 'Irlanda';
        });

        // Manejar los datos filtrados
        if (publicacionesIrlanda.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesIrlanda.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Irlanda"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Irlanda".</p>');
        }
    },
