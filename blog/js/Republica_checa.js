...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "República Checa"
        var publicacionesCheca = data.filter(function(publicacion) {
            return publicacion.Pais === 'Republica Checa';
        });

        // Manejar los datos filtrados
        if (publicacionesCheca.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesCheca.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "República Checa"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "República Checa".</p>');
        }
    },
