...
$.ajax({
    ...
    success: function(data) {
        // Filtrar las publicaciones por la categoría "Dinamarca"
        var publicacionesDinamarca = data.filter(function(publicacion) {
            return publicacion.Pais === 'Dinamarca';
        });

        // Manejar los datos filtrados
        if (publicacionesDinamarca.length > 0) {
            // Iterar sobre cada publicación en los datos filtrados
            publicacionesDinamarca.forEach(function(publicacion) {
            ...
            // Manejar el caso en que no haya publicaciones disponibles para la categoría "Dinamarca"
            $('#publicaciones-container').html('<p>No hay publicaciones disponibles para la categoría "Dinamarca".</p>');
        }
    },
