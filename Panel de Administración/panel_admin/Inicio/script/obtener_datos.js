...
fetch('/credenciales/credenciales.json')
    ...
    fetch('http://localhost/erasmus/backend/obtener_datos/obtener_paises_mas_likes.php')
    .then(response => response.json())
    .then(data => {
        if (data && Array.isArray(data)) {
            // Mostrar los resultados en el div con id "resultado"
            const resultadoDiv = document.getElementById('resultado');
            resultadoDiv.innerHTML = "<h3>Los 5 países con más likes:</h3>";

            for (let i = 0; i < Math.min(5, data.length); i++) {
                resultadoDiv.innerHTML += `<p>Pais ${i + 1}: ${data[i].País} - ${data[i].TotalLikes} likes</p>`;
            }
        } else {
            console.error("El formato de respuesta para los países con más likes no es el esperado.");
        }
    })
    .catch(error => {
        console.error(`Error al realizar la solicitud para los países con más likes: ${error}`);
    });
